...
def get_transalation_tot(text1,lang):
    final_text=[]
    try:
        text1= text1.replace('।','.')
        text1=text1.split('.')
        ...
        while i<len(text1):
            ...
            if len(otext)<100:
                ...
                print(otext,len(otext))
                transalation1= translator.translate(otext, dest=lang )
...
def get_transalation(text1,src,dest,multi):
    if multi:
        final_text=[]
        try:
            text1= re.sub(r'।|\n','.',text1)
            ...
            for txt in text1:
                if txt!='' and txt:
                    print(txt)
                    transalation1= translator.translate(txt+".", dest=dest,src=src )
                    eng_text= transalation1.text
                    print(eng_text)
                    final_text.append(eng_text)
        ...
        try:
            translator = Translator(service_urls=['translate.google.co.in', 'translate.google.com'], user_agent=ua, proxies=proxy_dict)
            transalation1= translator.translate(text1, dest=dest ,src=src)
            final_text= transalation1.text
            return final_text
...
def extract_5W1H(text,lang):
    text=re.sub(r'\.+','.',text)
    text=re.sub(r':','',text)
    eng_text=get_transalation(text,lang,"en",True)
    eng_text=re.sub(r'\.+','.',eng_text)
    doc=nlp(eng_text)
    ...
    cause_doc=cause_extractor(doc,eng_text)
    final_doc.update(cause_doc)
    method_doc=method_extractor(doc,eng_text)
    final_doc.update(method_doc)
    for key,value in final_doc.items():
        final_doc[key]=get_transalation(final_doc[key],"en",lang,False)
    return final_doc
