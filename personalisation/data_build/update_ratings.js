...
function get_custid_ratings() {
  MongoClient.connect(config.localMongodb.uri, function (err, db) {
    ...
    if (db != null) {
      db.collection(config.localMongodb.testing_users).find({ custid: 21118250, rating: { $in: [0, null] } }).toArray(function (err, docs) {
        ...
        if (!err && docs) {
          ...
          if (docs.length > 0) {
            ...
            async.eachSeries(docs, function (doc, asyncCb) {
              ...
              getFlippedPosts(custid, langid, datesArr, avg_time_spent,function (rtdata) {
                if (rtdata) {
                  db.collection(config.localMongodb.testing_users).updateOne({ custid: custid }, { $set: { rating: 1, last_post_date: rtdata.new_last_post_date ? rtdata.new_last_post_date : today_date, last_post_time: rtdata.new_last_post_time ? rtdata.new_last_post_time : "0" } }, { upsert: true }, function (err, success) {
                    // db.close();
                    if (!err) {
...
function getFlippedPosts(custid, langid, datesArr, avg_time_spent,userCb) {
  ...
  async.eachSeries(datesArr, (dateobj, asyncCb) => {
    ...
    if (langid) {
      ...
      executeAndCB(pushPostsQry, pushPostPrms, function (err, res) {
        if (!err) {
          pushPosts = res;
        } else {
          console.error(err);
                    ...
                    async.waterfall([
                      ...
                      (done) => {
                        ...
                        async.parallel({
                            ...
                            async.waterfall([
                              (done) => {
                                ...
                                if (finalPOstObj["events"] && finalPOstObj["events"].video_play_time >= config.ratingPoints.video_play_time) {
                                  ...
                                  points += 1
                                } else if (finalPOstObj.time_spent && finalPOstObj.time_spent != "-" && finalPOstObj.time_spent >= avg_time_spent && finalPOstObj.time_spent <100) {
                          ...
                          if ((finalPOstObj["events"] && finalPOstObj["events"].length>0)|| (finalPOstObj.time_spent && finalPOstObj.time_spent != "-" && finalPOstObj["time_spent"]<100)){
                            ...
                            if (db != null) {
                              db.collection(config.localMongodb.ratingCollection).findOne({ custid: custid, postid: item.postid }, {}, function (err, Doc) {
                                ...
                                if (Doc) {
                                  ...
                                  if(finalPOstObj.time_spent && finalPOstObj.time_spent != "-" && finalPOstObj["time_spent"]<100){
                                ...
                                db.collection(config.localMongodb.ratingCollection).updateOne({ custid: custid, postid: item.postid }, { $set: finalUserObj, $push: { reaction: finalPOstObj } }, { upsert: true }, function (err, res) {
                                  if (err) throw err;
