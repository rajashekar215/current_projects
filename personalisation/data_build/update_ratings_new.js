...
function get_custid_ratings() {
    MongoClient.connect(config.localMongodb.uri, function (err, localdb) {
        ...
        if (localdb != null) {
            localdb.collection(config.localMongodb.testing_users).find({ custid: 21118250, rating: { $in: [0, null] } }).limit(1).toArray(function (err, docs) {
                    ...
                    if (docs.length > 0) {
                        ...
                        async.eachSeries(docs, function (doc, asyncCb) {
                            ...
                            MongoClient.connect(config.liveMongodb.uri, function (err, livedb) {
                                ...
                                if (livedb != null) {
                                    get_mysqlConn(function (con1) {
                                        if (con1) {
                                            console.time("one custid")
                                            getFlippedPosts(con1,localdb, livedb, custid, langid, datesArr, function (rtdata) {
                                                if (rtdata) {
                            ...
                            console.log("custids ratings updated")
                            localdb.close();
                            // setTimeout(get_custid_ratings, 0);
                        })
                    } else {
                        console.log("no testing custids found")
                        // db.collection(config.localMongodb.testing_users).update({}, { $set: { rating:0} }, { upsert: true }, function (err, success) {
                        //     // db.close();
                        //     if (!err) {
...
function getFlippedPosts(con1,localdb, livedb, custid, langid, datesArr, userCb) {
    let new_last_post_date = ""
    let new_last_post_time = ""



    async.eachSeries(datesArr, (dateobj, asyncCb) => {
        // get_mysqlConn(function (con1) {
        //     if (con1) {
        ...
        if (langid) {
            ...
            executeAndCB('select', con1, pushPostsQry, pushPostPrms, function (err, res) {
                if (!err) {
                    pushPosts = res;
                } else {
                    new_last_post_date = date
                    new_last_post_time = time
                    console.error(err);
                    // process.exit(0);
                                    ...
                                    (done) => {
                                        ...
                                        async.parallel({
                                            getTags: (kwCb) => {
                                                ...
                                                executeAndCB('select', con1, tagQry, tagPrms, (err, tagRes) => {
                                            ...
                                            getCategory: (CtgryCb) => {
                                                ...
                                                executeAndCB('select', con1, ctgryQry, ctgeryPrms, (err, ctgryRes) => {
                                                ...
                                                async.waterfall([
                                                    (done) => {
                                                        if (pushPosts && pushPosts.length > 0 && pushPosts.include(item.postid)) {
                                                            ...
                                                            try {
                                                                executeAndCB('select', con1, query, params, function (err, sqlResponse) {
                                                        ...
                                                        if (finalPOstObj["events"] && finalPOstObj["events"].comments) {
                                                            points += config.ratingPoints.comments;
                                                        }
                                                        if (finalPOstObj["events"] && finalPOstObj["events"].addbookmark) {
                                                            points += config.ratingPoints.bookmark;
                                                        }
                                                        if (finalPOstObj["events"] && finalPOstObj["events"].full_img_ad_click) {
                                                            points += config.ratingPoints.full_img_ad_points;
                                                        }
                                                        if (finalPOstObj["events"] && finalPOstObj["events"].titleclick) {
                                                            points += config.ratingPoints.title_click;
                                                        }
                                                        if (finalPOstObj["events"] && finalPOstObj["events"].screenshots) {
                                                            points += config.ratingPoints.screenshots;
                                                        }
                                                        if (finalPOstObj["events"] && (finalPOstObj["events"].facebookshare || finalPOstObj["events"].whatsappshare || finalPOstObj["events"].twittershare || finalPOstObj["events"].othershare)) {
                                                            points += config.ratingPoints.shares;
                                                        }
                                                        if (finalPOstObj["isPushOpen"]) {
                                                            points += config.ratingPoints.push_open;
                                                        }
                                                        //  if(finalPOstObj["events"] && finalPOstObj["events"].category_click) {
                                                        //     points += config.ratingPoints.category_click;
                                                        // }
                                                        //  if(finalPOstObj["events"] && finalPOstObj["events"].topbuzz_click) {
                                                        //     points += config.ratingPoints.topbuzz_click;
                                                        // }
                                                        if (finalPOstObj["events"] && finalPOstObj["events"].likeclick_one) {
                                                            points += config.ratingPoints.likeclick_one;
                                                        }
                                                        if (finalPOstObj["events"] && finalPOstObj["events"].picdownload) {
                                                            points += config.ratingPoints.picdownload;
                                                        }
                                                        ...
                                                        if (finalPOstObj.time_spent && finalPOstObj.time_spent != "-") {
                                                            if (5 < finalPOstObj.time_spent && finalPOstObj.time_spent <= 10) {
                                                                points += 1;
                                                            }
                                                            else if (10 < finalPOstObj.time_spent && finalPOstObj.time_spent <= 15) {
                                                                points += 2;
                                                            }
...
function get_mysqlConn(conCb) {
    try {
        ...
        console.error('unkown error while fetch conn');
        console.error(cerr);
...
function executeAndCB(type, con1, query, data, eacb) {
    if (type === 'select') {
