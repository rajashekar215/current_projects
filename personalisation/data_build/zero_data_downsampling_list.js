                ...
                function fone(cat,fcb){
                    ...
                    localdb.collection("virality_all_data").find({new_cat:cat,target:0,train:null}).toArray(function(err,docs){
                        if(docs && docs.length){
                            ...
                            let split_docs=splitArray(docs, 10000);

                            async.eachSeries(split_docs,function(elem_doc,scb){
                                let text_list=[]
                                elem_doc.forEach(function(d){
                                    text_list.push(d["text"])
                                })
                                request.post(
                                    'http://49.156.128.11:5005/simapi',
                                    { json:  data = {"lang": "Telugu", "text1":d1, "text2":text_list,"le":text_list.length}  },
                                    function (error, response, body) {
                                        if (!error && response.statusCode == 200) {
                                            console.log(cat," body",body);
                                            // process.exit()
                                            if(body["simlist"] && body["simlist"].length>0){
                                                async.forEach(body["simlist"],function(s,mcb){
                                                    console.log("text1  ",d1)
                                                    console.log("text2  ",elem_doc[s[0]]["text"])
                                                    localdb.collection("virality_all_data").updateOne({_id:elem_doc[s[0]]._id},{$set:{train:0}},function(){
                                                        mcb();
                                                    })
                                                },function(){
