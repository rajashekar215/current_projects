...
def ordering(mdb,myresult,custid,from_time,skip,limit):
    print(skip,limit)
    client_103 = MongoClient('mongodb://49.156.128.102:27017/?readPreference=secondaryPreferred')
...
# =============================================================================
            ...
            if "longdesc" in post_doc and post_doc["longdesc"] and post_doc["longdesc"]!='':
                ...
                catg=catg+category(post_doc["longdesc"])
                row["ai_cat"]=catg
                s=s+" "+post_doc["longdesc"]
                sentiment=sia.polarity_scores(clean_text(post_doc["longdesc"]))["compound"]
                ...
                if "posttitle" in post_doc and post_doc["posttitle"] and post_doc["posttitle"]!='':
                    s=s+" "+post_doc["posttitle"]
                    sentiment=sia.polarity_scores(clean_text(post_doc["posttitle"]))["compound"]
                    row["sentiment"]=sentiment
...
# =============================================================================
    ...
    try:
        ...
        pred_prob=model.predict([np.array(final_sentence_matrix),np.array(senti_X)])
...
# =============================================================================
    ...
    for p in final_array:
        d_obj={}
        d_obj["postid"]=p["postid"] if "postid" in p else ""
        d_obj["posttitle"]=p["posttitle"] if "posttitle" in p else ""
        d_obj["longdesc"]=p["longdesc"] if "longdesc" in p else ""
        d_obj["imgurl"]=p["imgurl"] if "imgurl" in p else ""
        d_obj["type"]=p["type"] if "type" in p else ""
        d_obj["category_name"]=p["category_name"] if "category_name" in p else ""
        d_obj["is_breaking"]=p["is_breaking"] if "is_breaking" in p else ""
        d_obj["keywords"]=p["keywords"] if "keywords" in p else ""
        d_obj["ai_cat"]=p["ai_cat"] if "ai_cat" in p else ""
        d_obj["sentiment"]=p["sentiment"] if "sentiment" in p else ""
        d_obj["prediction"]=p["prediction"] if "prediction" in p else ""
        display_array.append(d_obj)
    return display_array
    
    
def quering(lang_id,categoryid,request_source,userEditions,userMandals,userVillages,singleDistrict,pageid,currentPostTime,from_time,to_time):
...
def recursive(already_length,from_time,to_time,lang_id,categoryid,request_source,userEditions,userMandals,userVillages,singleDistrict,pageid,currentPostTime,limit,custid,mdb):
    print(already_length,from_time,to_time)
    myresult=quering(lang_id,categoryid,request_source,userEditions,userMandals,userVillages,singleDistrict,pageid,currentPostTime,from_time,to_time)
    print(already_length,len(myresult),from_time,to_time)
    if already_length+len(myresult)>0:
        if pageid+limit<=already_length+len(myresult):
            res=ordering(mdb,myresult,custid,from_time,pageid-already_length,limit)
            print("condition1")
...
# =============================================================================
           ...
           print("condition3")
           result1=ordering(mdb,myresult,custid,from_time,pageid-already_length,0)
           if not len(result1)==0 and len(result1)<limit:
                print("condition3_1")
                to_time=from_time
                from_time=from_time-86400
                myresult2=quering(lang_id,categoryid,request_source,userEditions,userMandals,userVillages,singleDistrict,pageid,currentPostTime,from_time,to_time)
                result2=ordering(mdb,myresult2,custid,from_time,0,limit-len(result1))
                result1.extend(result2)
               ...
               return result1[:limit]
                ...
                to_time=from_time
                from_time=from_time-86400
                res=recursive(already_length+len(myresult),from_time,to_time,lang_id,categoryid,request_source,userEditions,userMandals,userVillages,singleDistrict,pageid,currentPostTime,limit,custid,mdb) 
                r_ids=  [d['postid'] for d in res]       
                print(r_ids)
                return res
    else:
...
# =============================================================================
    ...
    final_result= recursive(0,from_time,to_time,lang_id,categoryid,request_source,userEditions,userMandals,userVillages,singleDistrict,pageid,currentPostTime,limit,custid,mdb)
# =============================================================================
#     mycursor.close()
#     mydb.close()
# =============================================================================
