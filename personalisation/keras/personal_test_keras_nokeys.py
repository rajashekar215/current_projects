...
custid=22172222
documents=mdb.rating_points_testing_users.find({"custid" : custid,"date" :{"$nin":["2019-09-29","2019-09-30"]}},{"category":1,"keywords":1,"post_desc":1,"rating":1})
...
final_sentence_matrix=np.zeros((len(cleaned_text_X),2, EMBEDDING_DIM))        
...
mt=time.time()
model = Sequential()
model.add(Conv1D(64, 1, activation='selu',input_shape=(2,EMBEDDING_DIM)))
...
#custid=22172222
t4=time.time()
tlist=mdb.rating_points_testing_users.find({"custid" : custid,"date":{"$in":["2019-09-29","2019-09-30"]}},{"category":1,"keywords":1,"post_desc":1,"rating":1})
...
test_final_sentence_matrix=np.zeros((len(test_cleaned_text_X),2, EMBEDDING_DIM))        
