...
# =============================================================================
        ...
        mdb[active_users_collection].update_one({"custid" : custid},{'$set':{'model_tags_updated':1,'model_tags_uptime':time.time(),'model_tags_last_post_time':new_last_post_time,'avg_time_spent':new_avg_time_spent,"total_posts":new_total_posts}})
    else:
        print(str(custid)+" don't have atleast 1 flips")
        print(custid," last post time",new_last_post_time)
        mdb[active_users_collection].update_one({"custid" : custid},{'$set':{'model_tags_updated':1,'model_tags_uptime':time.time(),'avg_time_spent':new_avg_time_spent,"total_posts":new_total_posts}})
...
def user_model_update():
    ...
    if mdb[active_users_collection].count_documents({'model_tags_updated':{'$in':[None,0]}})>0:
        print("active users are fetched for model update")
        active_users=mdb[active_users_collection].find({'model_tags_updated':{'$in':[None,0]}}).limit(100)
        mode_gen_time={}
        ...
        for user in active_users:
            ...
            avg_time_spent=0
            total_posts=0
            if "model_tags_last_post_time" in user and user["model_tags_last_post_time"]:
                last_post_time=user["model_tags_last_post_time"]
        ...
        mdb[active_users_collection].update_many({},{'$set':{'model_tags_updated':0}})
