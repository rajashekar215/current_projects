...
#getTaggers('వెనీసుకు చెందిన వర్తకుడు నికొలో డా కాంటి భారతదేశం గుండా ప్రయాణిస్తూ, తెలుగు భాషలోని పదములు ఇటాలియన్ భాష వలె అజంతాలు (అచ్చు అంతమున కలిగి) గా ఉండటం గమనించి ', 'te')
...
custid=22172222
#documents=mdb.rating_points_testing_users.find({"custid" : custid},{"category":1,"keywords":1,"post_desc":1,"rating":1})
documents=mdb.rating_points_testing_users.find({"custid" : custid,"date" :{"$nin":["2019-10-04","2019-10-05"]}},{"category":1,"keywords":1,"post_desc":1,"rating":1})
...
mt=time.time()
model = Sequential()
model.add(Conv1D(64, 1, activation='selu',input_shape=(3,EMBEDDING_DIM)))
...
#model.add(Dense(100, activation='selu',input_shape=(EMBEDDING_DIM,)))
...
model.fit(final_sentence_matrix, labels, epochs=50, verbose=2)
tmt=time.time()-mt
print("time taken for train:: ",tmt)

et=time.time()
# evaluate the model
loss, accuracy = model.evaluate(final_sentence_matrix, labels, verbose=0)
print('Accuracy: %f' % (accuracy*100))
tet=time.time()-et
print("time taken for evaluation:: ",tet)
#model.save("../models_tags/"+str(custid))


pred_labels=model.predict_classes(final_sentence_matrix)
...
#custid=22172222
t4=time.time()
tlist=mdb.rating_points_testing_users.find({"custid" : custid,"date":{"$in":["2019-10-04","2019-10-05"]}},{"category":1,"keywords":1,"post_desc":1,"rating":1})
...
# =============================================================================
    ...
    test_key_X.append(clean_text(keys))
...
test_cleaned_text_X==test_text_X
test_cleaned_key_X=test_key_X
# =============================================================================
# test_x=[]
# test_y=[]
# 
...
for p in range(len(test_cleaned_text_X)):
    test_final_sentence_matrix[p][0]=np.nanmean(test_final_key_matrix[p],axis=0)
    test_final_sentence_matrix[p][1]=np.nanmean(test_final_cat_matrix[p],axis=0)
    test_final_sentence_matrix[p][2]=np.nanmean(test_final_text_matrix[p],axis=0)
