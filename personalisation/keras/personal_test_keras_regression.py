...
from keras.layers import Dense,Conv1D,Conv2D,MaxPooling1D,GlobalAveragePooling1D,Dropout,GlobalMaxPooling1D
...
documents=mdb.rating_points_test.find({"custid" : custid},{"category":1,"keywords":1,"post_desc":1,"rating":1})
docs=list(documents)
...
final_input_matrix=np.zeros((len(cleaned_X),200, EMBEDDING_DIM))        
i=0
for p in cleaned_X:
    awords=p.split(" ")
    j=0
...
model = Sequential()
model.add(Conv2D(128, 5, activation='relu',input_shape=(200,300)))
...
# 
# final_input_matrix_test=np.zeros((len(cleaned_test_X),200, EMBEDDING_DIM))        
# i=0
