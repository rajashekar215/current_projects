...
def ordering(mdb,mycursor,myresult,custid,from_time,skip,limit,lastpostid):
    print(skip,limit)
            ...
            if skip and limit:
                if unseen_length>0 and len(seen_posts)>0:
                    ...
                    if len(res)>=limit:
                        ...
                        if lastpostid:
                            res2=[]
                            last=f_ids.index(lastpostid)
                            print("lastpostid:::  ",lastpostid,last)
                            index = last+1
                            while limit>0:
                                res2.append(final_array[index])
                                index = (index + 1) % len(final_array)
                                limit=limit-1
                            res.extend(res2)
                            r_ids=  [d['postid'] for d in res]      
                            print("1c2",r_ids)
                ...
                if unseen_length>0 and len(seen_posts)>0:
                    ...
                    if lastpostid:
                        last=f_ids.index(lastpostid)
                        print("lastpostid:::  ",lastpostid,last)
                        r_ids=  [d['postid'] for d in final_array[last:]]       
                        print("1c9",r_ids)
                        return final_array[skip:]
                    else:
                        r_ids=  [d['postid'] for d in final_array[skip:]]       
...
def recursive(already_length,from_time,to_time,mycursor,lang_id,categoryid,request_source,userEditions,userMandals,userVillages,singleDistrict,pageid,currentPostTime,limit,custid,mdb,lastpostid):
    print(already_length,from_time,to_time)
    myresult=quering(mycursor,lang_id,categoryid,request_source,userEditions,userMandals,userVillages,singleDistrict,pageid,currentPostTime,from_time,to_time)
    print(already_length,len(myresult),from_time,to_time)
    if already_length+len(myresult)>0:
        if pageid+limit<=already_length+len(myresult):
            res=ordering(mdb,mycursor,myresult,custid,from_time,pageid-already_length,limit,lastpostid)
            print("condition1")
            r_ids=  [d['postid'] for d in res]       
            print(r_ids)
            return res
...
# =============================================================================
           ...
           print("condition3")
           result1=ordering(mdb,mycursor,myresult,custid,from_time,pageid-already_length,0,lastpostid)
           if not len(result1)==0 and len(result1)<limit:
               ...
               return result1[:limit]
                ...
                to_time=from_time
                from_time=from_time-86400
                res=recursive(already_length+len(myresult),from_time,to_time,mycursor,lang_id,categoryid,request_source,userEditions,userMandals,userVillages,singleDistrict,pageid,currentPostTime,limit,custid,mdb,lastpostid) 
                r_ids=  [d['postid'] for d in res]       
                print(r_ids)
                return res
    else:
...
def predictions(custid,lang_id,categoryid,request_source,userEditions,userMandals,userVillages,singleDistrict,pageid,limit,currentPostTime,lastpostid):
    pageid=int(pageid)
    limit=int(limit)
    ...
    final_result= recursive(0,from_time,to_time,mycursor,lang_id,categoryid,request_source,userEditions,userMandals,userVillages,singleDistrict,pageid,currentPostTime,limit,custid,mdb,lastpostid)
    mycursor.close()
    mydb.close()
    client.close()
    return final_result
