...
def get_sentence_vector(words,MAX_TEXT_LENGTH, EMBEDDING_DIM):
    ...
    return np.nanmean(text_array,axis=0)
...
query="select * from {0} as t1 where t1.post_gmt > {1} AND  t1.category_id!=0 and t1.category_id!=2 and t1.lang_id={2} and (t1.post_status='published' or t1.post_status='published1' or t1.post_status='published2' or t1.post_status='published3' or t1.post_status='published4') and (type='image' or type='fullimage' or type='video')".format(table,from_time,lang_id)
print(query)
...
mdb=client["user_word_cloud"]

user_docs=mdb["rating_points_testing_users"].aggregate([
    {"$match":{"postid" : {"$in":top_match_postids}}}
    ,{"$group":{"_id":"$custid"}}
    ,{
         "$lookup":{
        "from":"push_user_details",
        "localField": "_id",
        "foreignField": "custid",
        "as": "user_details"
        }
