...
def prepare_push_users(last_postid):
    EMBEDDING_DIM=300
    MAX_TEXT_LENGTH=60
    ...
    if len(myresult1)>0:
        ...
        table="mag_posts_home_new"
        lang_id=8
        week=str(date.today() - timedelta(days=30))
        ...
        for p in myresult1:
            ...
            top_match=sorted( [(x,i,myresult[i]["post_id"],myresult[i]["post_desc"]) for (i,x) in enumerate(cos_sim_result) if x>0.80], reverse=True )#[:3]
            ...
            if len(top_match_postids)>0:
                ...
                client = MongoClient('mongodb://w2nai_local_admin:w2naimongoadmin@49.156.128.11:27017/')
                ...
                if len(user_docs)>0:
                    ...
                    client_103 = MongoClient('mongodb://103.248.83.227:27017/')
